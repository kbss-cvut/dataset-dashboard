...
public class DatasetSourceDao extends BaseDao<dataset_source> {
    ...
    public JsonArray getSparqlSelectResult(final String queryName, final String sparqlEndpointUrl) {
        ...
        String result = sparqlAccessor.getSparqlResult(queryName, Collections.emptyMap(), sparqlEndpointUrl, null, "application/json");
        if (result != null) {
    ...
    public List<String> getAllNamedGraphsInEndpoint(final String sparqlEndpointUrl) {
        ...
        getSparqlSelectResult("query/get_sparql_endpoint_named_graph_datasetsources.rq", sparqlEndpointUrl).forEach((e) -> {
...
//                register(endpointUrl, null);
     ...
     */
    @Transactional("txManager")
    public List<dataset_descriptor> getDescriptors(final String datasetSourceId, final String descriptorType) {
        TypedQuery q = em.createNativeQuery("SELECT DISTINCT ?datasetDescriptor { ?vocDescriptionInstance a ?vocDescription ; ?vocHasSource ?datasetSource . ?datasetDescriptor ?vocInvHasDatasetDescriptor ?vocDescriptionInstance. ?datasetDescriptor a ?datasetDescriptorType }", dataset_descriptor.class);
     ...
     */
    @Transactional("txManager")
    public String getSparqlConstructResult(dataset_source ds,
        ...
        if (EntityToOwlClassMapper.isOfType(datasetSource,
            ...
            return sparqlAccessor.getSparqlResult(queryFile, bindings, endpointUrl,
